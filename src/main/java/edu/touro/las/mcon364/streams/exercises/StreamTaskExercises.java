...
public class StreamTaskExercises {
    ...
    public Map<Priority, List<String>> descriptionsByPriority(List<Task> tasks) {
        return tasks.stream()
                .collect(Collectors.groupingBy(Task::priority, Collectors.mapping(Task::description, Collectors.toList())));
    }
    ...
    public Map<Boolean, Long> countDonePartition(List<Task> tasks) {
        Map<Boolean, List<Task>> partitioned = partitionByDone(tasks);
